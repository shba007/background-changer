...
function App() {
  ...
  const [currentMode, setCurrentMode] = useState<'rgb' | 'hex' | 'hsl' | 'hsb'>(
    'hex'
  );
  ...
  return (
    ...
    >
      <div
        className={`${
          getColorLightness(currentColor)
            ...
            >
              {modes.map((mode) => (
                <li key={mode}>
                  <button
                    className={`${
                      mode === currentMode ? 'bg-slate-700 ' : ''
                    } group flex w-full items-center px-4 py-2 uppercase`}
                    onClick={() => {
                      setCurrentMode(mode);
